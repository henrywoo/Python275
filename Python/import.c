...
static const struct filedescr _PyImport_StandardFiletab[] = {
    //{".py", "U", PY_SOURCE},
    {".pyo", "rb", PY_COMPILED},//{".pyc", "rb", PY_COMPILED},
    {0, 0}
};
#endif
...
{
    //printf("%s path:%s name:%s type:%d\n",__func__,pathname, name, type);
    //int sz=strlen(pathname);
    //char pyopname[1024]={0};//TODO pyo file path name
    //if(strcmp(pathname+sz-3,".py")==0){
    //  sprintf(pyopname,"%so",pathname);
    //}
    //pathname = pyopname;
    //printf("%s path:%s name:%s\n",__func__,pathname, name);
    //if(type==PY_SOURCE){type=PY_COMPILED;}
